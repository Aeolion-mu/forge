...
> **不只是跑分玩具。** Forge 已用于真实开发——作为编程 Agent 去构建一个生产级 **飞书（Lark）自动化机器人**（OCR 抽取 → 分级规则引擎 → LLM 审核）。正是这次实战，暴露出大模型"没理清就动手"的毛病——也催生了下面的纪律机制。
...
四个框架：**同样 50 题、同一模型（`deepseek-v4-pro`）、同一官方 `run_evaluation` 评分器**，并**对思考强度做了控制**（见下）。

| 框架 | 思考强度 | 解决 / 50 | 解决率 | 空补丁 |
|---|---|---|---|---|
| **🔥 Forge（本仓库）** | **high — 与基线对齐** | **39 / 50** | **78%** | **0** |
| 🔥 Forge | max | 39 / 50 | 78% | 0 |
| `mini-swe-agent`（官方基线） | high（默认） | 37 / 50 | 74% | 0 |
| `hermes-agent` | high（默认） | 35 / 50 | 70% | 5 |
| Claude Code | max（自动，见下） | 35 / 50 | 70% | 3 |

**同 `high` 档排名：Forge 39 > mini 37 > hermes 35。**

#### 控制思考强度（为什么这是公平对比）

DeepSeek V4 Pro 默认开启思考。据官方文档：普通请求默认 `reasoning_effort=high`，而被它识别为 **Claude Code / OpenCode 的请求会自动提升到 `max`**。所以开箱即用时：Forge 与 Claude Code 跑在 `max`、mini/hermes 跑在 `high`。为消除这个 confound，**把 Forge 降到 `high`（对齐基线）重跑——结果还是 39/50。** 三点结论：

- **Forge 的成绩不是思考预算堆出来的**——max→high 一道没掉。
- **同为 `high` 档：Forge 39 > mini 37 > hermes 35。**
- **Claude Code 在 `max`（35）反而低于 mini 在 `high`（37）**——更多思考没帮上忙；框架质量才是决定因素。（Claude Code 是为 Claude 系模型调优的，不是 DeepSeek——"顶尖框架是模型相关的"。）

另外两点：

- **可信度 sanity check**：DeepSeek V4 Pro 官方全 500 题成绩 **80.6%**，Forge 在本子集 **78%** 紧贴其旁——佐证整条流水线（镜像 / 适配器 / 评分）无系统偏差、Forge 几乎榨出了模型的全部编程能力。
- **Forge 每题都产出了补丁（空补丁 0）**：收尾自审让 diff 更小但没误伤产出；其他框架偶有"引导不出可用 diff"的空补丁。
