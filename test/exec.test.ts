...
test("execSandboxed：已 aborted 的 signal → 不 spawn，立即返回 aborted=true（code=130）", async () => {
  ...
  ac.abort();
  const t0 = Date.now();
  const r = await execSandboxed("sleep 30", { cwd: process.cwd(), timeoutMs: 5000, signal: ac.signal });
...
test("execSandboxed：运行中 abort → 立刻 kill 子进程并返回 aborted=true", async () => {
  ...
  const r = await execSandboxed("sleep 30", { cwd: process.cwd(), timeoutMs: 10000, signal: ac.signal });
...
test("execSandboxed：无 signal 时与原行为一致（命令正常跑完）", async () => {
  const r = await execSandboxed("echo hello", { cwd: process.cwd(), timeoutMs: 5000 });
